...
//
// API "rankdb": static Resource Client
//
// Command:
// $ goagen
...
// DownloadDoc downloads /files with the given filename and writes it to the file dest.
...
func (c *Client) DownloadSwagger(ctx context.Context, filename, dest string) (int64, error) {
	...
	p := path.Join("/api/swagger/", filename)
	u := url.URL{Host: c.Host, Scheme: scheme, Path: p}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return 0, err
