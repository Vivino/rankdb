...
func HealthHealthOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.HealthController) (http.ResponseWriter, *app.RankdbSysinfo) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	var mt *app.RankdbSysinfo
	if resp != nil {
		var _ok bool
		mt, _ok = resp.(*app.RankdbSysinfo)
		if !_ok {
			t.Fatalf("invalid response media: got variable of type %T, value %+v, expected instance of app.RankdbSysinfo", resp, resp)
		}
	}
...
// HealthHealthUnauthorized runs the method Health of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
...
// RootHealthOK runs the method Root of the given controller with the given parameters.
...
func RootHealthOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.HealthController) http.ResponseWriter {
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "HealthTest"), rw, req, prms)
	rootCtx, _err := app.NewRootHealthContext(goaCtx, req, service)
	if _err != nil {
		e, ok := _err.(goa.ServiceError)
		if !ok {
			panic("invalid test data " + _err.Error()) // bug
